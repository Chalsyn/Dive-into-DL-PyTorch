...
def train(net, data_loader, train_optimizer, temperature, estimator, tau_plus, beta):   #trian_optimizer是什么，data_loader?
    net.train()                            #初始化网络
    total_loss, total_num, train_bar = 0.0, 0, tqdm(data_loader)
    for pos_1, pos_2, target in train_bar:
        pos_1, pos_2 = pos_1.to(device,non_blocking=True), pos_2.to(device,non_blocking=True)   #non_blocking提高读写速度
        feature_1, out_1 = net(pos_1)                                                           #得到的特征和projector后的特征
        feature_2, out_2 = net(pos_2)

        loss = criterion(out_1, out_2, tau_plus, batch_size, beta, estimator)

        train_optimizer.zero_grad()     #optimizer已经设计好需要优化的参数，优化方法，学习率等
        loss.backward()
        train_optimizer.step()          #单次优化

        total_num += batch_size
        total_loss += loss.item() * batch_size      #item()取tensor值
...
def tes111(net, memory_data_loader, test_data_loader):
    net.eval()                                    
...
if __name__ == '__main__':
    ...
    train_loader = DataLoader(train_data, batch_size=batch_size, shuffle=True, num_workers=0, pin_memory=True, drop_last=True)  #锁内存
    ...
    model = nn.DataParallel(model)   #使用多个GPU训练
    ...
    # training loop
    path1=''.join(['../results/',dataset_name])
    ...
    for epoch in range(1, epochs + 1):
        ...
        if epoch % 10 == 0:
            test_acc_1, test_acc_5 = tes111(model, memory_loader, test_loader)
